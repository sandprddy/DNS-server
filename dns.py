...
# Taken from ENTS-641(Advanced networks and protocols)
...
# from ENTS 641(Advanced Networks and protocols)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='A Python local DNS Server')
    parser.add_argument(
        '-c', dest='path', action='store', required=True,
        help='Configuration file')
    parser.add_argument(
        '-i', dest='iface', action='store', default='127.0.0.1', required=False,
        help='IP address where the DNS server should be running')
    parser.add_argument(
    ...
    parser.add_argument(
        '--noforward', dest='noforward', action='store', default=False, required=False,
    ...
    rules = RuleEngine2(path)  # converts the file to rules
    ...
    try:
        ...
        print ">> Could not start server -- is another program on same port"
        exit(1)
